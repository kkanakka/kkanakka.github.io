...
<table>
  ...
  <tr><td><b>SSE</b> (server-sent events)</td><td>One‑way server → client streaming. Resume semantics via <code>Last-Event-ID</code>, survives proxies, plain HTTP. <b>But <code>EventSource</code> is GET‑only</b> — with POST you implement the reconnect loop yourself.</td><td>The client needs to push mid‑stream.</td><td><a href="/docs/aire/llm-chat">llm-chat</a>, <a href="/docs/aire/developer-api">developer-api</a>, <a href="/docs/aire/desktop-chat-frontend">desktop-chat-frontend</a></td></tr>
...
<div class="trap"><b>The line that scores:</b> "SSE, because the data only flows one way and <code>Last-Event-ID</code> lets a dropped connection resume instead of regenerating — which for an LLM response is the difference between a hiccup and paying twice for the same answer. Worth noting the browser's <code>EventSource</code> is GET‑only, so on a POST endpoint I'd track the last id and drive the reconnect myself, and the server has to buffer the output for resume to mean anything."</div>

<h4>Which id is which</h4>
<p>Several identifiers are usually in flight at once and they are <em>not</em> interchangeable — conflating them produces duplicate messages and lost work:</p>
<table>
  <tbody><tr><th>Identifier</th><th>Generated by</th><th>Job</th></tr>
  <tr><td>Resource id (conversation, job)</td><td><b>Client</b>, up front</td><td>Addressing — and it lets the UI render before any round trip</td></tr>
  <tr><td>Idempotency key / message id</td><td><b>Client</b>, per action</td><td><b>Dedupe retries</b> — only the client knows a retry is the <em>same</em> action</td></tr>
  <tr><td>Request id</td><td><b>Server</b>, per call</td><td>Tracing and support</td></tr>
  <tr><td>Stream event id</td><td><b>Server</b>, per event</td><td>Resume cursor — transport state, dead when the stream ends</td></tr>
</tbody></table>
<p><b>The failure test</b> tells you they are different layers: lose the conversation id and you lost the chat; lose the event id and you re‑receive a few tokens. Full treatment in <a href="/docs/aire/llm-chat">llm-chat</a>.</p>
