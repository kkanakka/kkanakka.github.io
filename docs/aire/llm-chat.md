...
## Identifiers: which id, generated by whom {#lc-ids}

<p>Open the network tab on any chat product and you will see several identifiers moving at once. They live at different layers, have different lifetimes, and fail in different ways — and "which ids exist and who generates them" is a question interviewers ask precisely because conflating them produces duplicate messages and lost work.</p>

<table>
  <tbody><tr><th>Identifier</th><th>Generated by</th><th>Lifetime</th><th>Its one job</th></tr>
  <tr><td><b>Conversation id</b> (UUID)</td><td><b>Client</b>, when you click "new chat"</td><td>Forever — it is in the URL and the database</td><td>Addressing: which thread this is</td></tr>
  <tr><td><b>Message id</b> / <code>client_message_id</code> (UUID)</td><td><b>Client</b>, per send</td><td>Per message, permanent once stored</td><td><b>Idempotency</b>: dedupe a retried send</td></tr>
  <tr><td><b>Request id</b> (<code>req_01H…</code>)</td><td><b>Server</b>, per HTTP call</td><td>Per request; kept in logs</td><td>Tracing and support</td></tr>
  <tr><td><b>SSE event id</b></td><td><b>Server</b>, per streamed event</td><td>One stream; meaningless after it ends</td><td><b>Resume</b>: the cursor for <code>Last-Event-ID</code></td></tr>
</tbody></table>

<h4>Why the client generates the first two</h4>

<ul>
  <li><b>Optimistic UI.</b> The client routes to <code>/c/&lt;uuid&gt;</code>, renders the thread and shows the message immediately, with no round trip first. If the server minted the id, every new chat would stall on the network before anything appeared on screen.</li>
  <li><b>Idempotency — the real reason.</b> You send, the connection hangs, the client retries. With server‑generated ids you now have two messages, or two conversations. <b>Only the client knows that a retry is the <em>same</em> action</b>; the server sees two indistinguishable requests. A unique constraint on <code>(conversation, sender, client_message_id)</code> then enforces it at the database rather than by convention.</li>
  <li><b>Offline composition.</b> A message written with no connectivity needs an identity before the server has ever heard of it, so it can sit in a durable outbox and flush on reconnect (see <a href="/docs/aire/desktop-chat-frontend">desktop-chat-frontend</a>).</li>
  <li><b>Why a UUID works:</b> 122 bits of randomness makes collision a non‑issue, so a client can mint one unilaterally with no coordination.</li>
</ul>

<h4>Application state versus transport state</h4>

<p>The conversation id is <b>application state</b> — it means something to the product and outlives every connection. The SSE event id is <b>transport state</b> — a cursor into one stream, meaningless the moment that stream ends. The cleanest way to see they are different layers is how they fail:</p>

<ul>
  <li><b>Lose the conversation id</b> → you have lost the chat.</li>
  <li><b>Lose the event id</b> → you re‑receive a few tokens, or at worst regenerate.</li>
</ul>

## The streaming contract, mechanically {#lc-sse}

<p>The design leans on "resume instead of regenerate", so it is worth being precise about how that actually works — and about the part that is <em>not</em> free.</p>
...
retry: 3000            # server tells the client how long to wait before reconnecting
</code></pre>

<ul>
  <li><b>The server decides which events carry <code>id:</code>.</b> The client stores the most recent one it has seen and sends it back as a <code>Last-Event-ID</code> request header <b>once per reconnection attempt</b> — not per event, and never at all if the connection holds.</li>
  <li><b>An event without an <code>id:</code> does not reset anything.</b> The last‑event‑id buffer keeps its previous value, so ids can be emitted sparsely and resume still works from the last one seen.</li>
  <li><b>How often to emit ids is a real trade‑off.</b> Per event gives exact resume for ~10 bytes of overhead; every N events means replaying up to N−1 duplicates. For token streaming, per event is usually right — the overhead is small next to the payload, and exact resume is the entire point.</li>
  <li><b>Resume requires the server to buffer.</b> "Reconnect with <code>Last-Event-ID</code>" only works because the chat service held the generated text; a client‑side design decision here constrains the server contract, and it is worth naming as such.</li>
</ul>

<div class="trap"><b>The caveat that catches people: <code>EventSource</code> is GET‑only.</b> The browser's built‑in <code>EventSource</code> tracks the last event id and re‑sends it automatically on reconnect — but it cannot issue a POST with a JSON body. A chat API is <code>POST /conversations/:id/messages</code>, so the client is on <code>fetch()</code> + <code>ReadableStream</code> instead, and <b>you implement the reconnect loop yourself</b>: track the last id, reconnect, send the header, resume. SSE gives you the <em>semantics</em> for free; with POST it does not give you the <em>mechanism</em>. Saying this out loud is the difference between "we'll use SSE, it reconnects automatically" and an answer that has actually been built.</div>

<p><b>Why SSE rather than WebSockets anyway:</b> the data flows one way, SSE is plain HTTP so it survives proxies that mangle upgrades, and the resume semantics exist in the protocol rather than needing to be invented. WebSockets earn their cost when the client must push mid‑generation — which for a chat turn it does not.</p>
